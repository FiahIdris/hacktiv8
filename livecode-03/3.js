...
function reportSelling(list) {
  ...
  for (var i = 0; i < list.length; i++) {
    if (result[list[i].brand] === undefined) {
      result[list[i].brand] = { customers: [], sum: 0, max: 0, min: list[0].qty, avg: 0 }
    }
    result[list[i].brand].customers.push(list[i].name)
    result[list[i].brand].sum += list[i].qty
    if (result[list[i].brand].max < list[i].qty) {
      result[list[i].brand].max = list[i].qty
    }
    if (result[list[i].brand].min > list[i].qty) {
      result[list[i].brand].min = list[i].qty
    }
    var average = result[list[i].brand].sum / result[list[i].brand].customers.length
    result[list[i].brand].avg = average
